...
def test_qwen_vllm_initialization(llm_config: LLMConfig) -> None:
    """Test QwenVllm initialization."""
    with patch("llm_facade.qwen3.print") as mock_print, \
         patch("llm_facade.qwen3.OpenAI", autospec=True) as mock_openai_class:
        ...
        mock_print.assert_called_once_with(f"""VLLM client initialized:
              url: {llm_config.openai_api_base_url}
              key: {llm_config.openai_api_key}""")

        # Check that the client and config are set correctly
        assert vllm.client == real_instance_or_mock
...
def test_qwen_vllm_metadata(llm_config: LLMConfig) -> None:
    ...
    with patch("llm_facade.qwen3.OpenAI"), patch("llm_facade.qwen3.print"):
        vllm = QwenVllm(config=llm_config)
...
def test_qwen_vllm_complete(llm_config: LLMConfig, mock_openai: MagicMock) -> None:
    """Test QwenVllm complete method."""
    with patch("llm_facade.qwen3.OpenAI", return_value=mock_openai) as mock_openai_class:
        mock_logger = MagicMock()

    with patch("llm_facade.qwen3.print"):
